...
The command to package it as a wheel is:
   ...
   ./build_files/utils/make_maniblend_wheel.py ../build_linux_bpy/bin --output-dir=./
...
def parse_maniblend_version():
    #get path to this file
    script_dir = os.path.dirname(os.path.abspath(__file__))
    #get path to the blender source code
    blender_srcdir = os.path.abspath(os.path.join(script_dir, "..", ".."))
    changelog_path = os.path.join(blender_srcdir, "changelog.txt")
    with open(changelog_path, "r") as f:
        version_line = f.readline()
        version_split = version_line.split(" ")
        if len(version_split) != 3:
            sys.stderr.write(f"Failed to get maniblend version from string: {version_line}\n")
            sys.exit(1) 
        return version_split[2].strip()
...
def main() -> None:
    ...
    # Get Blender version.
    blender_version_str = str(parse_maniblend_version())
